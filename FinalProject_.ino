...
#define MIC_LOW 0.0 //Microphone min and max value
#define MIC_HIGH 753.0 
...
float fscale( float originalMin, float originalMax, float newBegin, float newEnd, float inputValue, float curve); //Imported from Arduino
...
int songmode = NORMAL; //different colors based on the mode
unsigned long song_avg; //Average sound measurement the last CYCLES
int iter = 0; //amount of iterations since the song_avg was reset
float fade_scale = 1.2; //speed LEDs fade to black unless relit
...
int avgs[AVGLEN] = {-1}; //shorter sound avg used to normalize the input values
...
void setup() {
  ...
  //fill average with low values to begin
...
//where brightness, mode, and time is read 
void loop() {
...
// Checks if the lamp should either enter a HIGH mode or back to NORMAL. 
// Enters HIGH mode if 30 or more sensor values are higher than 1.1xaverage over a few milliseconds
...
//visualizing the sounds to the lamp
void visualize_music() {
  ...
  //return if 0. Most likely garbage.
  if (sensor_value == 0)
    return;

  //return if reading deviates too much from past avg.
  ...
  for (int i = 0; i < NUM_LEDS; i++) 
    ...
    //Other LEDs begin to fade to black 
    } else {
...
//Compute average of array. Parameters are starting pointer and length
int compute_average(int *avgs, int len) {
  int sum = 0;
  for (int i = 0; i < len; i++)
    sum += avgs[i];
